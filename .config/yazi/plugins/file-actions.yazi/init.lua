...
function Popup.center_layout(area, height)
	...
	-- height 窗口区域高度
	local _, current_layout, _ = table.unpack(ui
		.Layout()
		-- 配置文件定义的布局
	...
	local _, centered_content_layout, _ = table.unpack(ui
		.Layout()
		-- 左中右
		:direction(ui.Layout.HORIZONTAL)
		:constraints({
	...
	local _, centered_ontent = table.unpack(ui
		.Layout()
		-- 上中没下
...
local entry = function(_, args)
	...
	local file_child, file_err =
		Command("file"):args({ "-bL", "--mime-type" }):args(sync_state.selected_files):stdout(Command.PIPED):spawn()
	...
	local action_child, action_err =
		Command("sh"):cwd(ya.quote(sync_state.actions_path)):args({ "-c", "ls -d */" }):stdout(Command.PIPED):spawn()
