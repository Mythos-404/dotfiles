...
function M:peek(job)
    ...
    if not child then return self:fallback_to_builtin(job) end
...
function M:fallback_to_builtin(job)
    ...
    if bound then ya.manager_emit("peek", { tostring(bound), only_if = tostring(job.file.url), upper_bound = "" }) end
end
