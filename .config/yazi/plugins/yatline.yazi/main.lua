...
function Yatline.string.get:cursor_percentage()
    ...
    local cursor = cx.active.current.cursor
    local length = #cx.active.current.files
    if cursor ~= 0 and length ~= 0 then percentage = math.floor((cursor + 1) * 100 / length) end
...
function Yatline.line.get:tabs(side)
    ...
    for i = 1, tabs do
        local text = i
        if tab_width > 2 then text = ya.truncate(text .. " " .. cx.tabs[i]:name(), { max = tab_width }) end
        ...
        if i == cx.tabs.idx then
            ...
            if style_a.bg ~= "reset" or show_background then
                ...
                if show_background then separator_style.bg = style_c.bg end
            ...
            if i == cx.tabs.idx - 1 then
                ...
                if style_a.bg ~= "reset" or (show_background and style_c.bg ~= "reset") then
                    if not show_background or (show_background and style_c.bg == "reset") then
                        ...
                        if show_background then separator_style.fg = style_c.bg end
...
return {
    setup = function(_, config)
        ...
        if display_header_line == nil then display_header_line = true end

        local display_status_line = config.display_status_line
        if display_status_line == nil then display_status_line = true end
        ...
        if config.theme then config = config.theme end
        ...
        Progress.redraw = function(self)
            ...
            if progress.total == 0 then return { config_text(self._area) } end
            ...
            local percent = 99
            if progress.found ~= 0 then percent = math.min(99, ya.round(progress.processed * 100 / progress.found)) end
        ...
        Root.layout = function(self)
            local constraints = {}
            if display_header_line then table.insert(constraints, ui.Constraint.Length(1)) end

            table.insert(constraints, ui.Constraint.Fill(1))

            if display_status_line then table.insert(constraints, ui.Constraint.Length(1)) end
        ...
        Root.build = function(self)
            ...
            if display_status_line then table.insert(childrens, Status:new(self._chunks[i], cx.active)) end
