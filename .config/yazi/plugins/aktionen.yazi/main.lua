...
PopupMenu._render = ya.sync(function(state, display, items, height, cursor)
    Root.redraw = function(self)
        if not display then return state.old_render(self) end
...
H.set_args = ya.sync(function(_, args)
    ...
    for _, arg in pairs(args) do
        if H.flags[arg] ~= nil then H.flags[arg] = true end
    end
    return H.flags
end)
...
H.sync_init = ya.sync(function(state)
    ...
    if hovered and not hovered.url.is_archive then table.insert(H.state.cursor_files, tostring(hovered.url)) end

    H.state.selected_files = {}
    for _, url in pairs(cx.active.selected) do
        if not url.is_archive then table.insert(H.state.selected_files, tostring(url)) end
    end
...
H.init_action = function()
    if #H.state.cursor_files == 0 and #H.state.selected_files == 0 then return end
    ...
    while true do
        ...
        if action.info.single and #H.state.selected_files ~= 1 then goto continue_get_action end

        if action.info.mime then
            if action.info.mime.disables then
                for _, minietype in ipairs(action.info.mime.disables) do
                    if selected_mimetype_set[minietype] then goto continue_get_action end
                end
            end
            if action.info.mime.enables and #action.info.mime.enables ~= 0 then
                ...
                for selected_mimetype in pairs(selected_mimetype_set) do
                    if not mime_enable_set[selected_mimetype] then goto continue_get_action end
                end
            end
        end
