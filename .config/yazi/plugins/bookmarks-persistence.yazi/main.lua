...
local function delete_lines_by_content(file_path, pattern)
    ...
    for line in file:lines() do
        if not line:find(pattern) then table.insert(lines, line) end
    end
    file:close()
...
local load_file_to_state = ya.sync(function(state, filename)
    ...
    for line in file:lines() do
        ...
        if bookmark == nil or #bookmark < 4 then goto nextline end
...
local save_bookmark = ya.sync(function(state, message, key)
    ...
    for _, cand in ipairs(state.bookmarks) do
        if tostring(under_cursor_file.url) == cand.desc then return end
    end

    -- if input message is empty,set message to file url
    if message == "" then message = under_cursor_file.url end
...
local auto_generate_key = ya.sync(function(state)
    ...
    for _, key in ipairs(SUPPORTED_KEYS) do
        ...
        for _, cand in ipairs(state.bookmarks) do
            if key == cand.on then goto continue end
        end
...
return {
    entry = function(_, args)
        ...
        if action == "delete_all" then return delete_all_bookmarks() end
