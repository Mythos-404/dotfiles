...
mp.register_event("end-file", function()
    ...
    if timer2 ~= nil then timer2:kill() end
    seekNumber = 0
    currentIndex = 0
    undoRedo = 0
    seconds = 0
...
local function undo()
    if (filePath ~= nil) and (countTimer >= 0) and (countTimer < 0.6) and (seeking == 0) then
        ...
        if currentIndex < 0 then
            if osd_messages == true then mp.osd_message("No Undo Found") end
            currentIndex = 0
            msg.info("No undo found")
        else
            if seekTable[currentIndex] < 0 then seekTable[currentIndex] = 0 end
            ...
            undoRedo = 1
            if osd_messages == true then mp.osd_message("Undo") end
...
local function redo()
    if (filePath ~= nil) and (currentIndex < seekNumber) then
        ...
        if osd_messages == true then mp.osd_message("Redo") end
        msg.info("Seeked using redo")
    elseif (filePath ~= nil) and (currentIndex == seekNumber) then
        if osd_messages == true then mp.osd_message("No Redo Found") end
...
mp.add_key_binding("ctrl+Z", "undoCaps", redo)
