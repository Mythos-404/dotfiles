...
function table:filter(filter)
    ...
    for index, value in ipairs(self) do
        if filter(index, value) then nt = table.push(nt, value) end
    end
    return nt
end
...
function command_split(command)
    ...
    if #temp then result = table.push(result, temp) end
    ...
    for event, cmd in pairs(Instance.on) do
        if type(cmd) == "table" then
            for index, cmd_part in ipairs(cmd) do
                if type(cmd_part) == "table" then Instance.on[event][index] = table.concat(cmd_part, " ") end
            end
...
function InputEvent:emit(event)
    if self.ignored[event] then
        if now() - self.ignored[event] < self.duration then return end
    ...
    if event == "release" and self.on[event] == "ignore" then event = "release-auto" end

    if event == "repeat" and self.on[event] == "ignore" then event = "click" end
...
function InputEvent:handler(event)
    ...
    if event == "down" then self:ignore("repeat") end
...
function InputEvent:exec()
    ...
    for _, v in ipairs(event_pattern) do
        if self.on[v.to] then queue_string = queue_string:replace(v.from, v.to) end
    end
...
function InputEvent:rebind(diff)
    if type(diff) == "table" then self = table.assign(self, diff) end
...
function bind(key, on)
    ...
    if type(on) == "string" then on = utils.parse_json(on) end
...
function on_options_update(list)
    if list.configs then bind_from_options_configs() end
end
