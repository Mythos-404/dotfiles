...
if o.term_width_limit < 0 then o.term_width_limit = nil end
if o.term_height_limit < 0 then o.term_height_limit = nil end
...
local function scroll_hint(search)
    ...
    if search then hint = hint .. " 进行滚动浏览且可使用按键 " .. o.key_search end
    hint = hint .. " 执行搜索）"
...
local function get_kbinfo_lines(_)
...
local function append_filters(s, prop, prefix)
    ...
    for _, f in ipairs(mp.get_property_native(prop, {})) do
        local n = f.name
        if f.enabled ~= nil and not f.enabled then n = n .. " （禁用）" end
...
local function add_video_out(s)
    ...
    local scale = nil
    if not mp.get_property_native("fullscreen") then scale = mp.get_property_native("current-window-scale") end
...
local function term_ellipsis(s, maxwidth)
    ...
    for i = 1, #s do
        ...
        elseif x < 128 or x >= 192 then -- non UTF8-continuation
            ...
            if width > maxwidth then return s:sub(1, fit_len) .. ellipsis end
        end
    end
...
local function split(str, pat, plain)
    ...
    repeat
        ...
        r[i], i = sub(str, init --[[ @as integer ]], f0 and f0 - 1), i + 1
        init = f0 and f1 + 1
    until f0 == nil
    return r
end
...
local function keybinding_info(after_scroll, bindlist)
    ...
    if not kbinfo_lines or not after_scroll then kbinfo_lines = get_kbinfo_lines(o.term_width_limit) end
...
local function add_track(c, t, i)
    ...
    if track_rg or album_rg then append(c, "", { prefix = "回放增益：" }) end
    if track_rg then
...
local function opt_time(t)
    if type(t) == type(1.1) then return mp.format_time(t) end
    return "?"
end
...
local function update_scale(value)
    ...
    local scale = 1
    if not scale_with_video and value > 0 then scale = 720 / value end
