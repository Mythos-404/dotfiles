...
if utils.shared_script_property_set then utils.shared_script_property_set("simplehistory-menu-open", "no") end
...
function starts_protocol(tab, val)
    for index, value in ipairs(tab) do
        if val:find(value) == 1 then return true end
    end
    return false
end
...
function contain_value(tab, val)
    ...
    for index, value in ipairs(tab) do
        if value.match(string.lower(val), string.lower(value)) then return true end
    end
...
function has_value(tab, val, array2d)
    ...
    if array2d then
        for i = 1, #tab do
            if tab[i] and string.lower(tab[i][array2d]) == string.lower(val) then return true end
        end
    end
...
function format_time(seconds, sep, decimals, style)
    ...
    if decimals == "truncate" then
        s = math.floor(s)
        decimals = 0
        if style == "timestamp" then seconds = math.floor(seconds) end
    end
...
function get_list_contents(filter, sort)
    ...
    if filter == "fileonly" then
        for i = 1, #list_contents do
            if tonumber(list_contents[i].found_time) == 0 then table.insert(filtered_table, list_contents[i]) end
        end
    ...
    if filter == "titleonly" then
        for i = 1, #list_contents do
            if list_contents[i].found_title then table.insert(filtered_table, list_contents[i]) end
        end
    ...
    if filter == "playing" then
        for i = 1, #list_contents do
            if list_contents[i].found_path == filePath then table.insert(filtered_table, list_contents[i]) end
        end
    ...
    if sort ~= current_sort then list_sort(list_contents, sort) end
...
function draw_list()
    ...
    if search_active and not list_contents[1] then osd_msg = osd_msg .. "No search results found" .. osd_msg_end end
    ...
    if list_start > 0 and not showall then osd_msg = osd_msg .. o.list_sliced_prefix .. osd_msg_end end
    for i = list_start, list_start + o.list_show_amount - 1, 1 do
        ...
        if o.show_item_number then osd_index = (i + 1) .. ". " end
...
function list_empty_error_msg()
    ...
    msg.info(msg_text)
    if o.osd_messages == true and not list_drawn then mp.osd_message(msg_text) end
end
...
function display_list(filter, sort, action)
    ...
    if not has_value(o.filters_and_sequence, filter) then table.insert(o.filters_and_sequence, filter) end
    ...
    for i = 1, #list_pages do
        ...
        if list_pages[i][1] ~= filter then list_pages[i][3] = 0 end
    ...
    draw_list()
    if utils.shared_script_property_set then utils.shared_script_property_set("simplehistory-menu-open", "yes") end
...
function list_move_up(action)
    ...
    if search_active and o.search_not_typing_smartly then list_search_not_typing_mode(true) end
end
...
function get_page_properties(filter)
    ...
    if list_cursor > #list_contents then list_move_last() end
end
...
function select_filter_sequence(pos)
    ...
    for i = 1, #o.filters_and_sequence do
        if filterName == o.filters_and_sequence[i] then curr_pos = i end
    end
    ...
    if o.filters_and_sequence[target_pos] then display_list(o.filters_and_sequence[target_pos], nil, "hide-osd") end
end
...
function get_list_keybinds()
    ...
    if not search_active then bind_keys(o.list_close_keybind, "list-close", list_close_and_trash_collection) end
...
function update_search_results(character, action)
    ...
    if action == "string_del" then search_string = search_string:sub(1, -2) end
    search_string = search_string .. character
    local prev_contents_length = #list_contents
    get_list_contents(filterName)

    if prev_contents_length ~= #list_contents then list_highlight_cursor = {} end
...
function unbind_search_keys()
    ...
    if not search_active then unbind_keys(o.list_close_keybind, "search_exit") end
end
...
function write_log(target_time, update_seekTime, entry_limit)
    ...
    if target_time then seekTime = target_time end
    if seekTime < 0 then seekTime = 0 end
    ...
    if not update_seekTime then seekTime = prev_seekTime end
end

function history_incognito_mode()
    if not incognito_mode then
        incognito_mode = true
        if o.osd_messages == true then mp.osd_message("🕵 Incognito Mode Enabled") end
        ...
        if o.osd_messages == true then mp.osd_message("Incognito Mode Disabled") end
...
function history_save(target_time)
    if filePath ~= nil then
        if history_blacklist_check() then return end
...
function history_resume()
    ...
    elseif filePath ~= nil then
        ...
        if seekTime > 0 then
            ...
            if o.osd_messages == true then mp.osd_message("No Resume Position Found For This Video") end
