...
function load_file_index_in_current_directory(index)
    ...
    if serialized and serialized.dirname then
        ...
        if files[index] then mp.commandv("loadfile", join_path(serialized.dirname, files[index])) end
    end
end
...
function select_current_chapter()
    ...
    if current_chapter_index ~= new_chapter_index then
        ...
        if itable_has(config.top_bar_flash_on, "chapter") then Elements:flash({ "top_bar" }) end
    end
end
