...
function Element:register_disposer(disposer)
    if not itable_index_of(self._disposers, disposer) then self._disposers[#self._disposers + 1] = disposer end
end
...
function Element:add_key_binding(key, fnFlags, namespace)
    ...
    names[name] = true
    if type(fn) == "string" then fn = self:create_action(fn) end
