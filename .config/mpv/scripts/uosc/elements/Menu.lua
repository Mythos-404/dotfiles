...
function Menu:destroy()
    ...
    if utils.shared_script_property_set then utils.shared_script_property_set("uosc-menu-type", nil) end
...
function Menu:activate_selected_item(shortcut, is_pointer)
    ...
    if item then
        -- Is submenu
        if item.items then
            if not self.mouse_nav then self:select_index(1, item.id) end
...
function Menu:move_selected_item_by(delta)
    ...
    if current_index and items_count > 1 then
        ...
        if current_index ~= new_index then self:move_selected_item_to(new_index) end
    end
end
...
function Menu:render()
    ...
    local function draw_menu(menu, x, pos)
        ...
        if draw_title then
            ...
            if menu.title and not menu.ass_safe_title then menu.ass_safe_title = ass_escape(menu.title) end
            ...
            if menu.search then
                ...
                if is_current and requires_submit then
                    ...
                    if get_point_to_rectangle_proximity(cursor, icon_rect) == 0 then icon_opacity = menu_opacity end
                end
