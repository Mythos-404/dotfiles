...
function itable_find(itable, compare, from, to)
    ...
    for index = from, to, from < to and 1 or -1 do
        if index > 0 and index <= #itable and compare(itable[index], index) then return index, itable[index] end
    end
end
...
function itable_slice(itable, start_pos, end_pos)
    ...
    local new_table = {}
    for index, value in ipairs(itable) do
        if index >= start_pos and index <= end_pos then new_table[#new_table + 1] = value end
    end
    return new_table
end
