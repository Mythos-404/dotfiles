...
function open_command_menu(data, opts)
    ...
    local function callback(event)
        ...
        elseif event.type == "activate" then
            ...
            if not event.modifiers and not event.action then run_command(event.value) end
            -- Convention: Only pure item activations should close the menu.
            -- Using modifiers or triggering item actions should not.
            if not event.keep_open and not event.modifiers and not event.action then menu:close() end
        end
    end
    ...
    return function()
        ...
        function remove_or_delete(index, value, menu_id, modifiers)
            if opts.on_remove and opts.on_delete then
                ...
                if handler then handler({ type = method, value = value, index = index }) end
            ...
            if event.type == "activate" then
                if (event.action == "move_up" or event.action == "move_down") and opts.on_move then
                    ...
                    if to_index >= 1 and to_index <= #menu.current.items then
                        ...
                        if not event.is_pointer then menu:scroll_to_index(to_index, nil, true) end
                    end
        ...
        if opts.active_prop then mp.observe_property(opts.active_prop, "native", handle_active_prop_change) end
    end
end
    ...
    local function handle_activate(event)
        if event.value == "{load}" then
            ...
            if snd and (event.action == "as_secondary" or event.modifiers == "shift") then
                ...
                if snd.enable_prop then mp.commandv("set", snd.enable_prop, "yes") end
            ...
            elseif not event.modifiers or event.modifiers == "alt" then
                ...
                if opts.enable_prop then mp.commandv("set", opts.enable_prop, "yes") end
            end
        end
    end
...
function open_file_navigation_menu(directory_path, handle_activate, opts)
    ...
    local function serialize_items(path, selected_path)
        ...
        if not serialized then return {}, 0, "Couldn't serialize path \"" .. path .. "." end
        local files, directories, error = read_directory(serialized.path, {
            types = opts.allowed_types,
            hidden = options.show_hidden_files,
        })
        if error then return {}, 1, error end
    ...
    menu = Menu:open(menu_data, function(event)
        ...
        elseif event.type == "key" then
            if event.id == "right" then
                ...
                if selected_item then activate(table_assign({}, selected_item, { type = "activate" }), true) end
...
function open_open_file_menu()
    ...
    local function handle_file_loaded()
        if menu and menu:is_alive() then menu:activate_one_value(normalize_path(mp.get_property_native("path"))) end
    end
...
function create_track_loader_menu_opener(opts)
    ...
    return function()
        ...
        if not path then path = options.default_directory end
...
function open_subtitle_downloader()
    ...
    handle_download = function(data)
        ...
        menu = Menu:open({
            ...
            if event.type == "key" and event.key == "enter" then menu:close() end
        end)
        ...
        call_ziggy_async(args, function(error, data)
            ...
            if
                should_abort(error, data, function(data)
