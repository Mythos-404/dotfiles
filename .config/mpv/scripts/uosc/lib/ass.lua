...
function ass_mt.opacity(self, opacity, fraction)
    ...
    local text = ""
    if opacity.main then text = text .. string.format("\\alpha&H%X&", opacity_to_alpha(opacity.main * fraction)) end
    if opacity.primary then text = text .. string.format("\\1a&H%X&", opacity_to_alpha(opacity.primary * fraction)) end
    if opacity.border then text = text .. string.format("\\3a&H%X&", opacity_to_alpha(opacity.border * fraction)) end
    if opacity.shadow then text = text .. string.format("\\4a&H%X&", opacity_to_alpha(opacity.shadow * fraction)) end
    if self == nil then
        return text
    elseif text ~= "" then
