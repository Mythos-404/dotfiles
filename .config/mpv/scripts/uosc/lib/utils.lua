...
function get_line_to_line_intersection(lax, lay, lbx, lby, max, may, mbx, mby)
    ...
    if uA >= 0 and uA <= 1 and uB >= 0 and uB <= 1 then return lax + (uA * (lbx - lax)), lay + (uA * (lby - lay)) end
...
function get_ray_to_rectangle_distance(ax, ay, bx, by, rect)
    -- Is inside
    if ax >= rect.ax and ax <= rect.bx and ay >= rect.ay and ay <= rect.by then return 0 end
        ...
        if item ~= "." and item ~= ".." and (opts.hidden or item:sub(1, 1) ~= ".") then
            ...
            if info then
                if info.is_file then
                    if not opts.types or has_any_extension(item, opts.types) then files[#files + 1] = item end
                else
...
function decide_navigation_in_list(paths, current_index, delta)
    ...
    if state.shuffle then
        ...
        for index, value in ipairs(paths) do
            if not itable_has(trimmed_history, value) then shuffle_pool[#shuffle_pool + 1] = index end
        end
...
function delete_file_navigate(delta)
    ...
    if navigate_item(delta) then
        if state.has_playlist then mp.commandv("playlist-remove", playlist_pos - 1) end
    else
        mp.command("stop")
    end

    if is_local_file then
        if Menu:is_open("open-file") then Elements:maybe("menu", "delete_value", path) end
...
function normalize_chapters(chapters)
    ...
    for index, chapter in ipairs(chapters) do
        ...
        if chapter_number then chapter.title = t("Chapter %s", tonumber(chapter_number)) end
...
function render()
    ...
    -- submit
    if osd.res_x == display.width and osd.res_y == display.height and osd.data == ass.text then return end
